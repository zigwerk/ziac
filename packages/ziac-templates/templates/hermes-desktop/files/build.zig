...
pub fn build(b: *std.Build) void {
    ...
    stack.addImport("zigeffect_std", ziac_dep.module("zigeffect_std"));
    ...
    var test_options = std.Build.TestOptions{ .name = "project-tests", .root_module = stack, .test_runner = .{ .path = ziac_dep.module("zigeffect_test_runner").root_source_file.?, .mode = .server } };
    ...
    b.step("test", "Run deterministic project tests").dependOn(&run_tests.step);
}
