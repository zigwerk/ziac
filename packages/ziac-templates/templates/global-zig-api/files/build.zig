...
pub fn build(b: *std.Build) void {
    ...
    var test_options = std.Build.TestOptions{ .name = "app-tests", .root_module = app, .test_runner = .{ .path = ziac_dep.module("zigeffect_test_runner").root_source_file.?, .mode = .server } };
    ...
    b.step("test", "Run deterministic application tests").dependOn(&run_tests.step);
}
