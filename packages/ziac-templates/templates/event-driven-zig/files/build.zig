...
pub fn build(b: *std.Build) void {
    ...
    const tests = b.addTest(.{ .name = "worker-tests", .root_module = app, .test_runner = .{ .path = ziac_dep.module("zigeffect_test_runner").root_source_file.?, .mode = .server } });
    b.step("test", "Run deterministic worker tests").dependOn(&b.addRunArtifact(tests).step);
}
