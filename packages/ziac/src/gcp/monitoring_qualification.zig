...
pub const schema = "ziac.gcp.monitoring-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "uptime_endpoint_not_probed",
        "alert_incident_not_observed",
        "notification_delivery_not_observed",
        "slo_time_series_not_observed",
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "monitoring", .stage = "local-qualification", .created_at_millis = 0 });
