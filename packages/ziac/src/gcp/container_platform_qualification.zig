...
pub const schema = "ziac.gcp.container-platform-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "gke_control_plane_not_probed",
        "fleet_membership_not_observed",
        "function_invocation_not_probed",
        "batch_execution_not_observed",
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    if (evidence.created != resource_count or evidence.imported != resource_count or evidence.no_op != resource_count or evidence.deleted != resource_count) return error.InvalidQualificationEvidence;
    const graph_digest = try plan.desiredGraphDigestAlloc(allocator, graph);
    const graph_hex = std.fmt.bytesToHex(graph_digest, .lower);
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "container-platform", .stage = "local-qualification", .created_at_millis = 0 });
