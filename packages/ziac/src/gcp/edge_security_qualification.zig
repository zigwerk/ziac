...
pub const schema = "ziac.gcp.edge-security-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "dns_propagation_not_observed",
        "certificate_active_state_not_observed",
        "cache_hit_not_probed",
        "armor_enforcement_not_probed",
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    if (evidence.created != resource_count or evidence.imported != resource_count or evidence.no_op != resource_count or
        evidence.retained != retained_count or evidence.deleted != resource_count - retained_count) return error.InvalidQualificationEvidence;
    const graph_digest = try plan.desiredGraphDigestAlloc(allocator, graph);
    const graph_hex = std.fmt.bytesToHex(graph_digest, .lower);
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "edge-security", .stage = "local-qualification", .created_at_millis = 0 });
