...
pub const schema = "ziac.gcp.connectivity-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "vpn_handshake_not_observed",
        "bgp_routes_not_observed",
        "ncc_routes_not_observed",
        "psc_endpoint_not_probed",
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "connectivity", .stage = "local-qualification", .created_at_millis = 0 });
