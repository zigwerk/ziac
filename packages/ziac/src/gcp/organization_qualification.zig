...
pub const schema = "ziac.gcp.organization-foundation-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "destructive_hierarchy_actions_not_exercised",
        "billing_account_not_changed",
        "organization_policy_not_inferred",
        "workload_costs_excluded",
    },
};
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    if (evidence.created != graph.resources.items.len or evidence.imported != graph.resources.items.len or
        evidence.no_op != graph.resources.items.len or evidence.retained != retained or evidence.resumable_operations < 3) return error.InvalidQualificationEvidence;
    const graph_digest = try plan.desiredGraphDigestAlloc(allocator, graph);
    const graph_hex = std.fmt.bytesToHex(graph_digest, .lower);
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "organization-foundation", .stage = "local-qualification", .created_at_millis = 0 });
