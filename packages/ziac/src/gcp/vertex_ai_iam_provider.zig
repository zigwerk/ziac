...
pub const Handler = struct {
    ...
    pub fn supports(node: resource.ResourceNode) bool {
        return isVertexIam(node.type_name);
    }
    ...
    pub fn read(self: Handler, context: *provider_mod.OperationContext, node: resource.ResourceNode, _: ?[]const u8) ProviderError!provider_mod.ReadResult {
        ...
        const target = try requiredResolvedString(context, node.inputs, "resource");
...
fn isVertexIam(type_name: []const u8) bool {
    return std.mem.eql(u8, type_name, "gcp.vertex.DatasetIamMember") or
        std.mem.eql(u8, type_name, "gcp.vertex.ModelIamMember") or
        std.mem.eql(u8, type_name, "gcp.vertex.FeatureGroupIamMember") or
        std.mem.eql(u8, type_name, "gcp.vertex.FeatureOnlineStoreIamMember") or
        std.mem.eql(u8, type_name, "gcp.vertex.FeatureViewIamMember");
}
...
fn requiredResolvedString(context: *provider_mod.OperationContext, source: value.Value, name: []const u8) ProviderError![]const u8 {
...
fn regionalBaseAlloc(allocator: std.mem.Allocator, base: []const u8, location: []const u8) ProviderError![]u8 {
    const prefix = "https://";
    if (!std.mem.startsWith(u8, base, prefix) or std.mem.indexOfAny(u8, location, ".:@/ \t\r\n") != null) return error.InvalidConfiguration;
