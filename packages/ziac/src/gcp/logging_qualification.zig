...
pub const schema = "ziac.gcp.logging-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "log_route_not_probed",
        "sink_writer_iam_not_exercised",
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "logging", .stage = "local-qualification", .created_at_millis = 0 });
