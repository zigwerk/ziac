...
fn apiFor(node: resource.ResourceNode) ?client_mod.Api {
    if (std.mem.eql(u8, node.type_name, "gcp.eventarc.MessageBusIamMember") or
        std.mem.eql(u8, node.type_name, "gcp.eventarc.PipelineIamMember") or
        std.mem.eql(u8, node.type_name, "gcp.eventarc.EnrollmentIamMember") or
        std.mem.eql(u8, node.type_name, "gcp.eventarc.GoogleApiSourceIamMember")) return .eventarc;
    if (std.mem.eql(u8, node.type_name, "gcp.connectors.ConnectionIamMember")) return .connectors;
    return null;
}
