...
pub const schema = "ziac.gcp.kms-secret-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "irreversible_actions_not_exercised",
        "secret_payloads_not_recorded",
        "cryptographic_operations_not_exercised",
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    if (evidence.created != resource_count or evidence.imported != resource_count or evidence.no_op != resource_count or
        evidence.retained != retained or evidence.reversible_transitions < 2) return error.InvalidQualificationEvidence;
    const graph_digest = try plan.desiredGraphDigestAlloc(allocator, graph);
    const graph_hex = std.fmt.bytesToHex(graph_digest, .lower);
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "kms-secret", .stage = "local-qualification", .created_at_millis = 0 });
