...
pub const schema = "ziac.gcp.cloud-deploy-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "release_creation_not_exercised",
        "rollout_progression_not_exercised",
        "underlying_cloud_build_not_exercised",
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "cloud-deploy", .stage = "local-qualification", .created_at_millis = 0 });
