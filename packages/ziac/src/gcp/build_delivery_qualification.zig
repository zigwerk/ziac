...
pub const schema = "ziac.gcp.build-delivery-qualification.v1";
...
const Receipt = struct {
    ...
    limitations: []const []const u8 = &.{
        "not_authenticated",
        "scm_webhook_not_exercised",
        "build_execution_not_exercised",
        "artifact_upload_not_exercised",
...
pub fn serializeLocalAlloc(allocator: std.mem.Allocator, graph: *const resource.ResourceGraph, evidence: Evidence) !SerializedReceipt {
    ...
    var artifact = try visual_artifact.serializeAlloc(allocator, graph, null, .{ .stack = "build-delivery", .stage = "local-qualification", .created_at_millis = 0 });
