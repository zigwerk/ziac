#!/usr/bin/env bash
set -euo pipefail

schema="ziac.gcp.edge-security-qualification.v1"
ziac_bin="${ZIAC_BIN:-ziac}"
stack="${ZIAC_EDGE_STACK:-secure-edge}"
stage="${ZIAC_EDGE_STAGE:-qualification}"
import_stage="${stage}-import"

required=(
  ZIAC_EDGE_WORKSPACE
  ZIAC_LIVE_PROJECT
  ZIAC_EDGE_BACKEND_BUCKET
  ZIAC_EDGE_DNS_AUTHORIZATION
  ZIAC_EDGE_CERTIFICATE
  ZIAC_EDGE_CERTIFICATE_MAP
  ZIAC_EDGE_MAP_ENTRY
  ZIAC_EDGE_HTTPS_PROXY
  ZIAC_EDGE_CACHE_URL
  ZIAC_EDGE_DENY_URL
)
missing=()
for variable in "${required[@]}"; do
  if [[ -z "${!variable:-}" ]]; then missing+=("${variable}"); fi
done
for command in "${ziac_bin}" gcloud curl jq; do
  if ! command -v "${command}" >/dev/null 2>&1; then missing+=("tool:${command}"); fi
done
if (( ${#missing[@]} > 0 )); then
  printf '{"schema":"%s","status":"skipped","authenticated":false,"reason":"missing_credentials_configuration_or_tools","missing":[' "${schema}"
  separator=""
  for item in "${missing[@]}"; do printf '%s"%s"' "${separator}" "${item}"; separator=","; done
  printf ']}\n'
  exit 77
fi
if [[ "${ZIAC_LIVE_PROJECT}" != *-ziac-disposable ]]; then
  printf '{"schema":"%s","status":"failed","authenticated":false,"reason":"project_is_not_disposable"}\n' "${schema}"
  exit 2
fi
if ! gcloud auth application-default print-access-token >/dev/null 2>&1; then
  printf '{"schema":"%s","status":"skipped","authenticated":false,"reason":"application_default_credentials_unavailable"}\n' "${schema}"
  exit 77
fi

authenticated=true
phase="service_enablement"
on_error() {
  exit_code=$?
  printf '{"schema":"%s","status":"failed","authenticated":%s,"phase":"%s"}\n' "${schema}" "${authenticated}" "${phase}"
  exit "${exit_code}"
}
trap on_error ERR

gcloud services enable compute.googleapis.com certificatemanager.googleapis.com storage.googleapis.com --project="${ZIAC_LIVE_PROJECT}" --quiet
workspace="$(cd "${ZIAC_EDGE_WORKSPACE}" && pwd)"
cd "${workspace}"
unset ZIAC_STATE_BUCKET
export ZIAC_GCP_PROJECT="${ZIAC_LIVE_PROJECT}"
mkdir -p .ziac/qualification

phase="plan_apply"
plan_path=".ziac/qualification/${stage}.plan.json"
plan_receipt="$("${ziac_bin}" plan --stack "${stack}" --stage "${stage}" --provider gcp --allow-live --live-test --out "${plan_path}" --json)"
plan_digest="$(printf '%s' "${plan_receipt}" | jq -r '.plan_digest // empty')"
test -n "${plan_digest}"
"${ziac_bin}" deploy --stack "${stack}" --stage "${stage}" --provider gcp --allow-live --live-test --plan "${plan_path}" --approve "${plan_digest}" --json >/dev/null

state_file=".ziac/state/${stack}/${stage}/resources.json"
test -f "${state_file}"
resource_count="$(jq '.resources | length' "${state_file}")"
test "${resource_count}" -gt 0

phase="edge_probes"
gcloud compute backend-buckets describe "${ZIAC_EDGE_BACKEND_BUCKET}" --project="${ZIAC_LIVE_PROJECT}" --format='value(enableCdn)' | grep -Fx True >/dev/null
gcloud certificate-manager dns-authorizations describe "${ZIAC_EDGE_DNS_AUTHORIZATION}" --location=global --project="${ZIAC_LIVE_PROJECT}" --format='value(dnsResourceRecord.data)' | grep -E '.+' >/dev/null
gcloud certificate-manager certificates describe "${ZIAC_EDGE_CERTIFICATE}" --location=global --project="${ZIAC_LIVE_PROJECT}" --format='value(managed.state)' | grep -Fx ACTIVE >/dev/null
gcloud certificate-manager maps entries describe "${ZIAC_EDGE_MAP_ENTRY}" --map="${ZIAC_EDGE_CERTIFICATE_MAP}" --location=global --project="${ZIAC_LIVE_PROJECT}" --format='value(certificates)' | grep -F "/certificates/${ZIAC_EDGE_CERTIFICATE}" >/dev/null
gcloud compute target-https-proxies describe "${ZIAC_EDGE_HTTPS_PROXY}" --project="${ZIAC_LIVE_PROJECT}" --format='value(certificateMap)' | grep -F "/certificateMaps/${ZIAC_EDGE_CERTIFICATE_MAP}" >/dev/null

headers="$(mktemp)"
trap 'rm -f "${headers}"' EXIT
curl -fsS "${ZIAC_EDGE_CACHE_URL}" >/dev/null
curl -fsS -D "${headers}" "${ZIAC_EDGE_CACHE_URL}" >/dev/null
awk 'BEGIN { IGNORECASE=1; found=0 } /^Age:[[:space:]]*[1-9][0-9]*/ { found=1 } END { exit(found ? 0 : 1) }' "${headers}"
deny_status="$(curl -sS -o /dev/null -w '%{http_code}' "${ZIAC_EDGE_DENY_URL}")"
test "${deny_status}" = "403"

phase="second_state_import"
while IFS=$'\t' read -r resource_id physical_id; do
  test -n "${resource_id}"
  test -n "${physical_id}"
  "${ziac_bin}" import --stack "${stack}" --stage "${import_stage}" --provider gcp --allow-live --live-test --resource "${resource_id}" --id "${physical_id}" --json >/dev/null
done < <(jq -r '.resources[] | select(.physical_id != null) | [.resource_id, .physical_id] | @tsv' "${state_file}")
import_plan=".ziac/qualification/${import_stage}.plan.json"
import_receipt="$("${ziac_bin}" plan --stack "${stack}" --stage "${import_stage}" --provider gcp --allow-live --live-test --out "${import_plan}" --json)"
test "$(printf '%s' "${import_receipt}" | jq '.create + .update + .delete')" -eq 0
test "$(printf '%s' "${import_receipt}" | jq '.noop')" -eq "${resource_count}"

phase="destructive_cleanup"
if [[ "$(jq '[.resources[] | select(.protect == true or .retain_on_delete == true)] | length' "${state_file}")" -ne 0 ]]; then
  printf '{"schema":"%s","status":"failed","authenticated":true,"reason":"qualification_stack_is_not_cleanup_enabled"}\n' "${schema}"
  exit 2
fi
"${ziac_bin}" destroy --stack "${stack}" --stage "${stage}" --provider gcp --allow-live --live-test --confirm --json >/dev/null

trap - ERR EXIT
rm -f "${headers}"
jq -cn --arg schema "${schema}" --arg project "${ZIAC_LIVE_PROJECT}" --arg graph_digest "${plan_digest}" --argjson resources "${resource_count}" \
  '{schema:$schema,status:"passed",authenticated:true,project:$project,graph_digest:$graph_digest,resource_count:$resources,dns_authorization:"observed",certificate:"active",proxy_attachment:"observed",cdn_cache_hit:"observed",armor_deny:"passed",import_plan:"noop",cleanup:"deleted",cost_origin:"configuration_estimate"}'
