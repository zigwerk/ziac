...
test "GCP provider catalog is valid and covers every managed live type" {
    ...
    try std.testing.expectEqual(@as(usize, 253), managed_count);
}
...
test "GCP provider catalog exposes current and next-tranche coverage honestly" {
    ...
    const gke_cluster = ziac.gcp.coverage.find("gcp.container.Cluster") orelse return error.TestExpectedEqual;
    try std.testing.expectEqualStrings("M72", gke_cluster.milestone);
    try std.testing.expectEqual(ziac.gcp.coverage.Service.container, gke_cluster.service);
    try std.testing.expect(gke_cluster.capabilities.update);
    try std.testing.expect(gke_cluster.capabilities.estate);
    try std.testing.expect(gke_cluster.capabilities.cost);

    const function = ziac.gcp.coverage.find("gcp.functions.FunctionV2") orelse return error.TestExpectedEqual;
    try std.testing.expectEqual(ziac.gcp.coverage.Service.cloud_functions, function.service);
    try std.testing.expect(function.capabilities.import_resource);
    try std.testing.expect(function.capabilities.estate);

    const batch_job = ziac.gcp.coverage.find("gcp.batch.Job") orelse return error.TestExpectedEqual;
    try std.testing.expectEqual(ziac.gcp.coverage.Service.batch, batch_job.service);
    try std.testing.expect(!batch_job.capabilities.update);
    try std.testing.expect(!batch_job.capabilities.estate);
    try std.testing.expect(batch_job.capabilities.visual);
    try std.testing.expect(batch_job.capabilities.cost);

    const alert_policy = ziac.gcp.coverage.find("gcp.monitoring.AlertPolicy") orelse return error.TestExpectedEqual;
    try std.testing.expectEqualStrings("M73", alert_policy.milestone);
    try std.testing.expectEqual(ziac.gcp.coverage.Service.monitoring, alert_policy.service);
    try std.testing.expect(alert_policy.capabilities.update);
    try std.testing.expect(alert_policy.capabilities.estate);
    try std.testing.expect(alert_policy.capabilities.visual);
    try std.testing.expect(alert_policy.capabilities.cost);

    const monitoring_service = ziac.gcp.coverage.find("gcp.monitoring.Service") orelse return error.TestExpectedEqual;
    try std.testing.expect(monitoring_service.capabilities.update);
    try std.testing.expect(!monitoring_service.capabilities.estate);
    try std.testing.expect(monitoring_service.capabilities.visual);
    try std.testing.expect(monitoring_service.capabilities.cost);

    const log_bucket = ziac.gcp.coverage.find("gcp.logging.Bucket") orelse return error.TestExpectedEqual;
    try std.testing.expectEqualStrings("M74", log_bucket.milestone);
    try std.testing.expectEqual(ziac.gcp.coverage.Service.logging, log_bucket.service);
    try std.testing.expect(log_bucket.capabilities.update);
    try std.testing.expect(log_bucket.capabilities.estate);
    try std.testing.expect(log_bucket.capabilities.visual);
    try std.testing.expect(log_bucket.capabilities.cost);

    const project_exclusion = ziac.gcp.coverage.find("gcp.logging.Exclusion") orelse return error.TestExpectedEqual;
    try std.testing.expect(project_exclusion.capabilities.update);
    try std.testing.expect(!project_exclusion.capabilities.estate);
...
test "GCP provider coverage reports are deterministic filterable and provenance pinned" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, json_first, "monitoring:v1") != null);
    try std.testing.expect(std.mem.indexOf(u8, json_first, "monitoring:v3") != null);
    try std.testing.expect(std.mem.indexOf(u8, json_first, "logging:v2") != null);
